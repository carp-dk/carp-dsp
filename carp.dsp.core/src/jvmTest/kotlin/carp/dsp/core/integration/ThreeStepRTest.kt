...
class ThreeStepRTest : IntegrationTestBase()
{
    ...
    fun setupTestData()
    {
        val descriptor = loadFixture( "three-step-r.yaml" )
        ...
        setupWorkspaceAssets(
            root,
            "scripts/step1_load.R" to "scripts/step1_load.R",
            "scripts/step2_transform.R" to "scripts/step2_transform.R",
            "scripts/step3_finalize.R" to "scripts/step3_finalize.R",
            "data/input.csv" to "inputs/input.csv"
        )
    }

    @Test
    fun `three step R pipeline executes successfully`()
    {
        // Plan structure
