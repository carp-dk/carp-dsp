 ...
 * - Unknown environment kinds throw [UnsupportedEnvironmentKindException]
 * - `in-process` tasks throw [UnsupportedTaskTypeException] (no domain type yet)
 *
...
)
{
    ...
    {
        // Import environments first (builds UUID lookup for step references)
        val ( environments, keyToUuid ) =
        ...
        workflowDescriptor.steps.forEach { stepDesc ->
            workflow.addComponent( importStepInternal( stepDesc, keyToUuid ) )
        }
     ...
     * @param keyToUuid Lookup: environment key → UUID (from EnvironmentImporter)
     * @return Domain Step
     */
    private fun importStepInternal(
        stepDescriptor: StepDescriptor,
        keyToUuid: Map<String, UUID>
    ): Step
    {
        // Step ID: explicit UUID, deterministic v5, or fallback random
        val stepId = stepDescriptor.id?.let { tryParseUuid( it ) }
        ...
        val envUuid = keyToUuid[stepDescriptor.environmentId]
            ?: tryParseUuid( stepDescriptor.environmentId )
            ?: UUID.randomUUID()

        return Step(
            metadata = MetadataImporter.importStepMetadata( stepId, stepDescriptor.metadata ),
            task = TaskImporter.importTask( stepDescriptor.task, workflowNamespace ),
            environmentId = envUuid,
            inputs = stepDescriptor.inputs.map { PortImporter.importInputPort( it, keyToUuid ) },
            outputs = stepDescriptor.outputs.map { PortImporter.importOutputPort( it ) },
        )
    }
}
