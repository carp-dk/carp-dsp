...
import carp.dsp.core.application.cwl.DspToCwlTranslator
...
object PackageBuilder {
     ...
     *
     * CWL is auto-generated by translating the descriptor's steps via [DspToCwlTranslator].
     * Steps that have no CWL equivalent (e.g. [carp.dsp.core.application.authoring.descriptor.InProcessTaskDescriptor])
     * A future release will package these as small kotlin executable or JS executables that can then be wired in
     * are skipped. If no steps can be translated, [WorkflowArtifactPackage.cwl] is null.
     * Multiple steps are emitted as separate YAML documents separated by `---`.
     *
    ...
    fun build(
        ...
        val cwl = DspToCwlTranslator.translate(descriptor)
            .takeIf { it.isNotEmpty() }
            ?.let { assets ->
                CwlTranslationAsset(
                    content = assets.joinToString("\n---\n") { it.content },
                    toolVersion = assets.first().toolVersion,
                )
            }
